...
session_start();
include('db.php');
// Check if the user is logged in as a Deputy Finance Officer
...
if (isset($_POST['move_button'])) {
    ...
    $check_query = "SELECT * FROM os_accepted_bills WHERE Bill_Number = '$billNumberToMove' AND Status = 'processing'";
    $check_result = mysqli_query($conn, $check_query);
    
    if (mysqli_num_rows($check_result) > 0) {
        echo "Bill with Bill Number $billNumberToMove has already been moved.";
    } else {
        // Update the bill's status to "processing" in the os_accepted_bills table
        $update_query = "UPDATE os_accepted_bills SET Status = 'processing' WHERE Bill_Number = '$billNumberToMove'";
        
        if (mysqli_query($conn, $update_query)) {
            echo "Bill with Bill Number $billNumberToMove has been moved to processing state.";
            
            // Insert the bill details into the dfo_accepted_bills table
            $insert_query = "INSERT INTO dfo_accepted_bills (Bill_Number, Vendor_Name, Vendor_Company, Vendor_Phone_Number, Amount, Department, Invoice_Date, Status, User_Id, Budget_Head, Actions, Remarks)
                             ...
                             FROM os_accepted_bills
                             WHERE Bill_Number = '$billNumberToMove'";
            
            if (mysqli_query($conn, $insert_query)) {
                echo "Bill details added to DFO's accepted bills.";
            } else {
...
if (isset($_POST['update_remarks_button'])) {
    ...
    // Update the remarks for the bill in the os_accepted_bills table
    $updateRemarks_query = "UPDATE os_accepted_bills SET Remarks = '$newRemarks' WHERE Bill_Number = '$billNumberToUpdateRemarks'";
...
if (isset($_POST['reject_button'])) {
    ...
    // Update the bill's status to "Rejected" in the os_accepted_bills table
    $updateRejectQuery = "UPDATE os_accepted_bills SET Status = 'Rejected' WHERE Bill_Number = '$billNumberToReject'";

    if (mysqli_query($conn, $updateRejectQuery)) {
        echo "Bill with Bill Number $billNumberToReject has been rejected and moved back to processing state.";
    } else {
        echo "Error rejecting bill: " . mysqli_error($conn);
    }
}

// Retrieve and display bills from the 'os_accepted_bills' table
$user_id = $_SESSION["user_id"];
$query = "SELECT * FROM os_accepted_bills WHERE Status IN ('Accepted', 'Moved')";
...
<html>
<head>
    <title>Deputy Finance Officer Dashboard</title>
        ...
        <div>
            ...
            <div class="dropdown">
                ...
                <div class="dropdown-content">
                    ...
                    <a href="office_superintendent_login.php">Logout</a>
                </div>
            </div>
        </div>
    </header>
...
<body>
    <h2>Deputy Finance Officer Dashboard</h2>
    <table>
        <tr>
            <th>ID</th>
            <th>Bill Number</th>
